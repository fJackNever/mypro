...
import {Form,Input,DatePicker, Card,Select, AutoComplete ,InputNumber,message, Icon, Button,Upload,Modal,notification } from 'antd';
...
class newBorrowCarNotes extends PureComponent {
  ...
  AddAccidentSubmit = ()=>{
    this.props.form.validateFields((err, values) => {
      if(!err){
        ...
        if(values.customer_name){ AddAccidentFormData.append('customer_name',values.customer_name);}
        if(values.responsible_party){AddAccidentFormData.append('responsible_party',values.responsible_party);}
        if(values.thirdparty_amount) {AddAccidentFormData.append('thirdparty_amount', values.thirdparty_amount);}
        if(values.own_amount) {AddAccidentFormData.append('own_amount', values.own_amount);}
        if(values.detail_record) {AddAccidentFormData.append('detail_record', values.detail_record);}
        if(values.comment) {AddAccidentFormData.append('comment', values.comment);}
        if(values.address) {AddAccidentFormData.append('address', values.address);}
        if(addimgfromDataUrl.length>0){AddAccidentFormData.append('vehicle_damage_img',JSON.stringify(addimgfromDataUrl));}
        if(addimgfromData.length>0){AddAccidentFormData.append('detail_comment',JSON.stringify(addimgfromData));}
        if(values.in_maintain_time){AddAccidentFormData.append('in_maintain_time',new Date(values.in_maintain_time._d).getFullYear()+'-'+(new Date(values.in_maintain_time._d).getMonth()+1)+'-'+new Date(values.in_maintain_time._d).getDate());}
        if(values.miantain_finsih_time){AddAccidentFormData.append('miantain_finsih_time',new Date(values.miantain_finsih_time._d).getFullYear()+'-'+(new Date(values.miantain_finsih_time._d).getMonth()+1)+'-'+new Date(values.miantain_finsih_time._d).getDate());}
        if(values.maintian_content) {AddAccidentFormData.append('maintian_content', values.maintian_content);}
  ...
  render() {
    ...
    return (
      <div>
        <Card title={<div>出险记录<span style={{color:'#f50',fontSize:'14px',marginLeft:'24px'}}>( 记录出险信息 )</span></div>} style={{marginBottom:24}}>
