...
import {Form,Input,DatePicker,message, Card,Select, InputNumber, Table, Radio, Button,AutoComplete,Badge,notification} from 'antd';
...
class newSignCarNotes extends PureComponent {
  ...
  AddRentRecordSubmit = ()=>{
    ...
    this.props.form.validateFields((err, values) => {
      if(!err){
        ...
        ARRFormData.append('total_amount',values.total_amount);
  ...
  TotalAmount = (value)=>{
    this.props.form.validateFields((err, values) => {
      if(!err){
        this.props.form.setFields({
          total_amount: {value:parseInt(values.rent_month_amount)*parseInt(value)+parseInt(values.manager_amount)+parseInt(values.basic_premium)+parseInt(values.other_basic_premium)+parseInt(values.deposit)}
        })
      }
    })
  }
          ...
          <Form className={styles.form} >
            <div className={styles.formDiv}>
              <FormItem
                ...
                label="租车类型"
              >
                <Select value={this.state.type} placeholder="请选择租车类型" onChange={this.selectType}  disabled={this.state.licensePlateNodisabled}>
              ...
              <FormItem {...formItemLayout} label="品牌车型">
                {getFieldDecorator('vehicle_template', {
                  ...
                  <Input disabled={this.state.licensePlateNodisabled} placeholder="品牌车型会自动联想" />
                )}
            ...
            <div className={styles.formDiv}>
              ...
              <FormItem {...formItemLayout} className={styles.formButtonDiv}>
                ...
                <div className={styles.TagBtnDiv}>
                  <Button loading={this.state.ButtonLoading} className={styles.formButton} onClick={this.AddRentRecordSubmit} type="primary">确认修改</Button>
                  <Button className={styles.formButton} style={{marginLeft:'15px'}} onClick={()=>window.open("/#/ShowContract?entity_id="+this.state.entity_id,"_blank")} type="primary">合同打印</Button>
                </div>
                }
              </FormItem>
            </div>
